...
export const ProfilePage = () => {
  ...
  const [totalVotesPages, setTotalVotesPages] = useState(1);
  const { refreshUser } = useAuth();

  const { data: votesData, isLoading } = useQuery({
    queryKey: ['userVotes', votesPage],
    queryFn: async () => {
      const offset = (votesPage - 1) * limit;
      const response = await getApiClient().get('/users/my-votes', { params: { limit, offset } });
  ...
  useEffect(() => {
    if (votesData?.data) {
      ...
      if (votesData.meta) {
        setTotalVotesPages(Math.ceil(votesData.meta.total / limit));
      }
    ...
    const fetchCreatedPolls = async () => {
      if (user?.role === 'vendor' || user?.role === 'admin') {
        try {
          ...
          const res = await pollsApi.getMyCreatedPolls({ limit, offset });
            ...
            <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
              <div className="flex-1 flex justify-between sm:hidden">
                <button
                  ...
                  className={`relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 ${votesPage === 1 ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                >
                  Précédent
                </button>
                <button
                  onClick={() => setVotesPage((p) => Math.min(totalVotesPages, p + 1))}
                  disabled={votesPage === totalVotesPages}
                  className={`ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 ${votesPage === totalVotesPages ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                >
                  Suivant
                </button>
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700">
                    Page <span className="font-medium">{votesPage}</span> sur <span className="font-medium">{totalVotesPages}</span>
                  </p>
                </div>
                <div>
                  <nav className="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                    <button
                      onClick={() => setVotesPage((p) => Math.max(1, p - 1))}
                      disabled={votesPage === 1}
                      className={`relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 ${votesPage === 1 ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                    >
                      <span className="sr-only">Précédent</span>
                      <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                    ...
                    <button
                      onClick={() => setVotesPage((p) => Math.min(totalVotesPages, p + 1))}
                      disabled={votesPage === totalVotesPages}
                      className={`relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 bg-white text-sm font-medium text-gray-500 hover:bg-gray-50 ${votesPage === totalVotesPages ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                    >
                      <span className="sr-only">Suivant</span>
                      <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
