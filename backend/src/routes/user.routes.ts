...
import { Vote, Poll, PollOption, User } from '../models/index.js';
...
router.get('/my-votes', protect, async (req: Request, res: Response) => {
  try {
    ...
    const formattedVotes = votes.rows.map(vote => {
      ...
      return {
        id: votePlain.id,
        question: votePlain.poll?.question || 'Unknown Poll',
        optionText: votePlain.option?.text || 'Unknown Option',
        votedAt: votePlain.createdAt,
        reward: votePlain.rewardAmount
      };
    });
